...
var resizePizzas = function(size) {
  ...
  function changePizzaSizes(size) {
    // created three variables outside of for loop
    var pizzaElements = document.getElementsByClassName('randomPizzaContainer'),
        dx = determineDx(pizzaElements[0], size),
        newWidth = (pizzaElements[0].offsetWidth + dx) + 'px';
...
function updatePositions() {
  ...
  // changed this variable to select items by class name
  var items = document.getElementsByClassName('mover');

  // added a new for loop to calculates phase and pushes it an array
  phases = [];
  for(var k = 0; k < 5; k++) {
    phases.push(Math.sin((document.body.scrollTop / 1250) + (k % 5)));
  }
  // updated this for to access the element in the phase array
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // changed the number of pizzas generated to depend on widnow height
  var rows = Math.round(window.screen.height / s),
      pizzaCount = rows * cols;

  // moved document.querySelector into its own variable, changed to get by element id, and moved out of loop
